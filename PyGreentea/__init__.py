...
class MakeDatasetOffset(object):
    ...
    def calculate_offset_bounds(self, dataset):
        ...
        minima_shifts = tuple(0 if bb[0] is not None else c - i for bb, i, c in zip(bounding_box, self.input_dims, self.context))
        maxima_shifts = tuple(-i if bb[1] is not None else -c for bb, i, c in zip(bounding_box, self.input_dims, self.context))
