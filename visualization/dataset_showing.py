...
matplotlib.use('Agg')  # must come before any additional imports from matplotlib
import matplotlib.pyplot as plt


def showme(dset, z):
    '''
    :param dset: dictionary with numpy arrays in keys: data, components, label, pred, predseg, and potentially others
    :param z: the cross section location along the z axis
    ...
    segments_as_color = label2rgb(gt_slc, bg_label=0)
    a.imshow(segments_as_color, alpha=0.15, interpolation='nearest', extent=true_label_extent)
    ...
    a.set_ylim(0, max_shape[2])
    a.imshow(raw_slc, cmap=plt.cm.get_cmap('gray'), interpolation='nearest', extent=image_extent)
    segments_as_color = label2rgb(seg_slc, bg_label=0)
    a.imshow(segments_as_color, alpha=0.15, interpolation='nearest', extent=pred_extent)
    ...
    a.set_ylim(0, max_shape[2])
    segments_as_color = label2rgb(seg_slc, bg_label=0)
    a.imshow(segments_as_color, interpolation='nearest', extent=pred_extent)
    ...
    if 'components_positive' in dset:
        ...
        segments_as_color = label2rgb(components_positive, bg_label=0)
        a.imshow(segments_as_color, interpolation='nearest', extent=true_label_extent)
